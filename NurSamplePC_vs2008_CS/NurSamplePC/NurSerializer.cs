            ...
            try
            {
                ...
                foreach (int antenna in hNur.EnabledAntennas)
                {
                    try
                    {
                        ...
                        if (dBm < 0)
                        {
                            antennaList.Add(string.Format("Antenna {0} = Reflected Power {1:0.0} dBm", antenna, dBm));
                        }
                        else
                        {
                            antennaList.Add(string.Format("Antenna {0} = Reflected Power {1:0.0} dBm. Make sure that the antenna is connected.", antenna, dBm));
                        }
                    }
                    catch (NurApiException)
                    {
